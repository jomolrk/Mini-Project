...
$trains=$_POST['trains'];
$sql = "SELECT t_code FROM trains WHERE tname = '$trains'";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);

        //$query="UPDATE tbl_login SET log_type='$row[user]' WHERE log_emailid='$row[log_emailid]';";
        if (mysqli_query($conn, $sql)) {

            $message = "please confirm your ticket";
            echo "<script type='text/javascript'>alert('$message');</script>";
            echo "<script language='javascript'>";
            echo 'window.location.href = "bookticket.php"';
...
<head>
	...
	<style type="text/css">
		...
		html { 
		  background: url(images/trabg.jpg) no-repeat center center fixed; 
